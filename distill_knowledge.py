...
class TrainManager(object):
	...
	def train(self):
		lambda_ = 0.5 # self.config['lambda_student']
		T = 4.0 # self.config['T_student']
		...
		for epoch in range(epochs):
			...
			if val_acc > best_acc:
				best_acc = val_acc
				self.save(epoch, name='{}_{}_{}.pth.tar'.format(self.name, trial_id, epochs))
...
if __name__ == "__main__":
	...
	teacher_model = None
	student_model = create_student_model(args.student, dataset, use_cuda=args.cuda)
	train_config = {
		'epochs': args.epochs,
		...
		'is_plane': not check_model(args.student),
		'trial_id': trial_id,
	...
	if args.teacher:
		teacher_model = create_student_model(args.teacher, dataset, use_cuda=args.cuda)
		if args.teacher_checkpoint:
			print("---------- Loading Teacher -------")
			teacher_model = load_checkpoint(teacher_model, args.teacher_checkpoint)
		else:
			print("---------- Training Teacher -------")
			train_loader, test_loader = load_CIFAR(num_classes)
